    ...
    public class BlogPostBuilder : ViewComponentBuilderBase<BlogPost, BlogPostBuilder>
    {
        ...
        public override string ToHtmlString()
        {
            ...
            if (!string.IsNullOrEmpty(_searchTerm))
                allPosts = allPosts.Where(s => s.Title.ToLower().Contains(_searchTerm.ToLower()));

            if (!string.IsNullOrEmpty(_userId))
                allPosts = allPosts.Where(s => s.UserId.Contains(_userId));

            if (!string.IsNullOrEmpty(_userName))
                allPosts = allPosts.Where(s => s.UserName.Equals(_userName, StringComparison.InvariantCultureIgnoreCase));
